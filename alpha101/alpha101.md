	...
	Alpha#8: (-1 * rank(((sum(open, 5) * sum(returns, 5)) - delay((sum(open, 5) * sum(returns, 5)), 10))))
		(sum(open, 5) * sum(returns, 5))：5天开盘价总和 ** 五天收益总和 ：主要关注的点在于价格冲击 5日开盘均价*5 ** 5日累计收益 但这里出现了一个问题 小价值股跟高价值股无法放在一起比较 价格冲击高的高价值股的分数 大于 价格冲击高的低价值股 
		 delay((sum(open, 5) * sum(returns, 5)), 10)：延迟10天的5天开盘价总和 ** 五天收益总和 
				这个alpha关注的点在于：它认为如果这个股票相对于10天价格冲击高 则更容易回落 如果相下冲击多 则更容易反弹。 有一个问题 这里只关注了价格冲击 而没有看量能 这里做一个变体加个限制：
				 (adv20 > volume)？(（(-1 * rank(((sum(open, 5) * sum(returns, 5)) - delay((sum(open, 5) * sum(returns, 5)), 10))))） :-1)
				回测效果很差 不考虑
